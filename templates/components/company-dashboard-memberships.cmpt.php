...
<div id="wpjb-main">
    ...
    <div class="wpjb wpjb-page-company-products">
        ...
        <?php if (!empty($result)) : foreach ($result as $pricing) : ?>
                ...
                <?php if ($pricing->is_active == 0 || !is_object($summary)) : continue;
                endif; ?>
                <?php $has_active = true; ?>
                ...
                <div class="wpjb-company-product">
                    <div class="wpjb-company-product-header jobeleon-normal-bg">
                        <h3><?php esc_html_e($pricing->title) ?></h3>
                        <div class="wpjb-company-product-status">
                            <!--span class="wpjb-bulb wpjb-bulb-active"><?php _e("Active", "jobeleon") ?></span-->
                            <?php if ($summary->days_left < 7) : ?>
                                <span class="wpjb-bulb wpjb-bulb-expiring" style="background-color: #D54E21"><?php _e("Expiring", "jobeleon") ?></span>
                            <?php endif; ?>
                            <?php if ($pricing->meta->is_recurring->value() && $sub_status->value == "-1") : ?>
                                <span class="wpjb-bulb wpjb-bulb-expiring" style="background-color: #D54E21"><?php _e("Canceled", "jobeleon") ?></span>
                            <?php endif; ?>
                        </div>
                    </div>
                    ...
                    <div class="wpjb-company-product-subheader">
                        <div class="wpjb-company-product-indicator">
                            ...
                            <?php else : ?>
                                <?php _e('Never Expires', "jobeleon"); ?>
                            <?php endif; ?>
                        </div>
                        <?php if ($pricing->meta->is_recurring->value() && $sub_status->value != "-1") : ?>
                            <div class="wpjb-company-product-indicator">
                                <span class="wpjb-glyphs wpjb-icon-arrows-cw"></span>
                                <?php printf(__('Next Payment: %s', "jobeleon"), date(wpjb_date_format(), $subscription->current_period_end)); ?>
                            </div>
                        <?php endif; ?>
                            ...
                            <?php foreach ($single_job as $id => $usage) : ?>
                                ...
                                <div class="wpjb-company-product-details-single">
                                    ...
                                    <?php if ($usage["status"] == "unlimited") : ?>
                                        <?php printf(__("Uses: Unlimited", "jobeleon")); ?>
                                    <?php else : ?>
                                        <?php printf(__('Uses: %1$d / %2$d', "jobeleon"), $summary->bundle[$product->price_for][$id]["used"], $usage["usage"]); ?>
                                    <?php endif; ?>
                                </div>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    ...
                    <div class="wpjb-company-product-actions">
                        <div class="wpjb-company-product-actions-left">
                            <?php if (!$pricing->meta->is_recurring->value()) : ?>
                                <a href="<?php esc_attr_e(wpjb_link_to("membership_purchase", $pricing)) ?>" class="wpjb-manage-action wpjb-glyphs wpjb-icon-arrows-cw"><?php _e("Rebuy Package", "jobeleon"); ?></a>
                            <?php endif; ?>

                            <?php if ($pricing->meta->is_recurring->value() && $sub_status->value != "-1") : ?>
                                <a href="<?php echo wpjb_link_to("membership") ?>?action=cancel&id=<?php echo $summary->id ?>" class="wpjb-manage-action wpjb-glyphs wpjb-icon-cancel"><?php _e("Cancel Subscription", "jobeleon"); ?></a>
                            <?php elseif ($pricing->meta->is_recurring->value() && $sub_status->value == "-1") : ?>
                                <?php _e("You already canceled this membership. It will be moved to archive, after expiration date.", "jobeleon"); ?>
                            <?php endif; ?>
                        ...
                        <div class="wpjb-company-product-actions-right">
                            ...
                            <?php if (isset($archive_summary)) : ?>
                                <a href="#" class="wpjb-manage-action wpjb-manage-action-more wpjb-glyphs wpjb-icon-menu"><?php _e("More", "jobeleon"); ?></a>
                            <?php endif; ?>
                        </div>
                    </div>
                    <div class="wpjb-company-product-additional">
                        <?php if (isset($archive_summary)) : ?>
                            <?php foreach ($archive_summary as $asummary) : ?>
                                <?php if ($asummary->started_at == '0000-00-00' && $asummary->expires_at == '0000-00-00') : continue;
                                endif; ?>
                                        ...
                                        <?php foreach ($single_job as $id => $usage) : ?>
                                            ...
                                            <div class="wpjb-company-product-details-single">
                                                ...
                                                <?php if ($usage["status"] == "unlimited") : ?>
                                                    <?php printf(__("Uses: Unlimited", "jobeleon")); ?>
                                                <?php else : ?>
                                                    <?php printf(__('Uses: %1$d / %2$d', "jobeleon"), $asummary->bundle[$product->price_for][$id]["used"], $usage["usage"]); ?>
                                                <?php endif; ?>
                                            </div>
                                        <?php endforeach; ?>
                                    <?php endif; ?>
                            ...
                            <?php endforeach; ?>
                        <?php else : ?>
                            <?php _e("There is no archive records for this membership", "jobeleon"); ?>
                        <?php endif; ?>

                        <?php do_action('wpjb_company_products_additional_details'); ?>
                    </div>
                </div>
        <?php endforeach;
        endif; ?>
        ...
        <?php if (!$has_active) : ?>
            <div class="wpjb-upload-ui">
                <div class="wpjb-upload-inner">
                    <span class="wpjb-upload-info"><?php _e("You do not have any active memberships.", "jobeleon"); ?></span>
                    <span><a href="<?php echo get_permalink(wpjb_conf('urls_link_membership_pricing')) ?>" class="wpjb-button"><?php _e("Buy Membership!", "jobeleon"); ?></a></span>
                </div>
            </div>
        <?php endif; ?>


        <h2><?php _e("Archived Memberships", "jobeleon"); ?></h2>
        <?php $archived = 0; ?>
        <?php if (!empty($result)) : foreach ($result as $pricing) : ?>
                <?php /* @var $pricing Wpjb_Model_Pricing */ ?>
                <?php $active_summary = Wpjb_Model_Membership::getPackageSummary($pricing->id, wpjb_get_current_user_id("employer")) ?>
                <?php if (is_object($active_summary)) : continue;
                endif; ?>
                <?php $archived++; ?>
                ...
                <?php if (!empty($summary)) : ?>
                    ...
                    <div class="wpjb-company-product">
                        <div class="wpjb-company-product-header" style="background-color: #707070;">
                            <h3><?php esc_html_e($pricing->title) ?></h3>
                            <div class="wpjb-company-product-status">
                                <a href="#" class="wpjb-manage-action wpjb-manage-action-more wpjb-glyphs wpjb-icon-menu" style="background-color: #fff;"><?php _e("More", "jobeleon"); ?></a>
                            </div>
                        </div>
        ...
        <?php if ($archived == 0) : ?>
            <?php _e("You do not have any archived memberships", "jobeleon"); ?>
        <?php endif; ?>
